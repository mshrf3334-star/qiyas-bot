# app.py — Qiyas Bot (Webhook/PTB v21)
# ------------------------------------
...
def ai_system_prompt(style:str, ei_enabled:bool) -> str:
    ...
    encouragement = ("اختم بجملة تشجيعية قصيرة تضيف دافعية للطالب."
                     if ei_enabled else
...
def get_ei(context): return context.user_data.get("ei", EI_DEFAULT)
def set_ei(context, value: bool): context.user_data["ei"] = bool(value)
...
# ======================================================
#         منع التكرار (يتذكر آخر N أسئلة لكل قسم)
# ======================================================
SEEN_LIMIT = 400
...
def gen_quant() -> Dict[str, Any]:
    ...
    if t == "pow":
        base = random.randint(2, 15); exp = random.choice([2, 3])
        val  = base ** exp
        q = f"قيمة {base}^{exp} = ؟"
...
def gen_iq() -> Dict[str, Any]:
    ...
    if k == "letter_seq":
        ...
        if max_start < 0: step, max_start = 1, len(AR_LETTERS) - 6
    ...
    # mix_ops: +a, *b, +a, *b ...
    a = random.randint(2,6); b = random.choice([2,3])
...
def fmt_progress(i:int, total:int) -> str:
    blocks = 10; fill = int((i/total)*blocks)
...
def session_get(context: ContextTypes.DEFAULT_TYPE, cat:str) -> QuizSession:
    ...
    s = store.get(cat)
    if s and isinstance(s, QuizSession): return s
...
async def send_next(update:Update, context:ContextTypes.DEFAULT_TYPE, cat:str, label:str):
    ...
    # توليد سؤال مع منع التكرار الحديث
    for _ in range(6):
        q = s.current()
        if not q:
            break
    ...
    if not q:
        ...
        context.user_data["sessions"].pop(cat, None); return
...
def mult_table(n:int, upto:int=12) -> str:
    rows = [f"📐 جدول ضرب {n}:"]
    for i in range(1,upto+1): rows.append(f"{n} × {i} = {n*i}")
    return "\n".join(rows)

def clean_number_only(text: str) -> Optional[int]:
    t = text.strip(); m = re.fullmatch(r"(-?\d+)", t)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "كمي" in t:
        context.user_data.get("sessions", {}).pop("quant", None); context.user_data["streak"] = 0
        await update.message.reply_text("سيبدأ اختبار الكمي (حتى ٥٠٠). بالتوفيق! 💪")
        await send_next(update, context, "quant", "قدرات كمي"); return
    if "لفظي" in t:
        context.user_data.get("sessions", {}).pop("verbal", None); context.user_data["streak"] = 0
...
async def cmd_ei_on(update: Update, context: ContextTypes.DEFAULT_TYPE):
    set_ei(context, True);  await update.message.reply_text("تم تفعيل الذكاء العاطفي ✅")

async def cmd_ei_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    set_ei(context, False); await update.message.reply_text("تم إيقاف الذكاء العاطفي ⛔️")

# ====== استلام الإجابة من الأزرار ======
async def cb_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query; await query.answer()
    m = re.fullmatch(r"ans\|(\d+)", (query.data or ""));  if not m: return
    ...
    s = session_get(context, cat); res = s.check(choice)
...
async def ask_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    q = None
    if txt.startswith("/ask_ai") and " " in txt: q = txt.split(" ", 1)[1].strip()
    elif update.message and update.message.reply_to_message:
        q = (update.message.reply_to_message.text or "").strip()
    if not q:
        await update.message.reply_text("اكتب سؤالك بعد الأمر:\n/ask_ai كيف أذاكر القدرات؟"); return
    if not AI_API_KEY:
        await update.message.reply_text("⚠️ لم يتم ضبط AI_API_KEY في الخادم (Render)."); return

    prefs = get_ai_prefs(context);  ei_enabled = get_ei(context)
...
async def cmd_ai_style(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prefs = get_ai_prefs(context); prefs["style"] = args[1]
...
async def cmd_ai_temp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = (update.message.text or "").split()
    if len(args) < 2: await update.message.reply_text("استخدم: /ai_temp 0.2 إلى 1.5"); return
    try:
        t = float(args[1]); 
        if not (0.0 <= t <= 1.5): raise ValueError()
    except:
        await update.message.reply_text("قيمة غير صالحة. اختر رقمًا بين 0.0 و 1.5"); return
    prefs = get_ai_prefs(context); prefs["temperature"] = t
...
async def cmd_ai_model(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2: await update.message.reply_text("استخدم: /ai_model اسم_الموديل (مثال: gpt-4o-mini)"); return
    prefs = get_ai_prefs(context); prefs["model"] = args[1].strip()
...
async def cmd_ai_diag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    if not AI_API_KEY:
        await update.message.reply_text("❌ AI_API_KEY غير مضبوط في Render."); return
    prefs = get_ai_prefs(context)
    try:
        client = OpenAI(api_key=AI_API_KEY, base_url=AI_BASE_URL) if AI_BASE_URL else OpenAI(api_key=AI_API_KEY)
        def _call():
