...
def gen_iq() -> Dict[str, Any]:
    ...
    if k == "letter_seq":
        ...
        if max_start < 0: step = 1; max_start = len(AR_LETTERS) - 6
    ...
    if k == "squares":
        s = random.randint(2,6); seq = [i*i for i in range(s, s+4)]; ans = (s+4)**2
...
def session_get(context: ContextTypes.DEFAULT_TYPE, cat: str) -> QuizSession:
    ...
    s = QuizSession(gen, limit); store[cat] = s; return s
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok: await update.message.reply_text(f"{a} × {b} = {a*b}"); return
...
async def ask_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = (update.message.text or ""); q = None
...
async def cmd_ai_temp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        t = float(args[1]); if not (0.0 <= t <= 1.5): raise ValueError()
    except Exception:
